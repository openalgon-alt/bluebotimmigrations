...
export function SchedulePopup() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-4 pointer-events-none">
      {/* Chat Bubble */}
      <div
        className={`transition-all duration-500 ease-in-out origin-bottom-right pointer-events-auto ${isOpen ? "opacity-100 scale-100 translate-y-0" : "opacity-0 scale-90 translate-y-4 shadow-none pointer-events-none"}`}
      >
        <div className="bg-white/95 backdrop-blur-xl border border-primary/20 shadow-[0_20px_50px_-15px_rgba(var(--primary-rgb),0.3)] rounded-[32px] rounded-br-sm p-6 w-[calc(100vw-2rem)] sm:w-[360px] relative overflow-hidden group/card">
          ...
          <div className="flex flex-col gap-5 mt-1 relative z-10">
            ...
            <div className="flex flex-col gap-4 min-h-[60px]">
              ...
              {step === 1 && (
                <div
                  className="bg-primary/5 border border-primary/10 rounded-2xl rounded-tl-sm px-4 py-3.5 shadow-sm inline-flex items-center justify-center gap-1.5 animate-fade-up self-start h-11"
              ...
              {step >= 2 && (
                <div
                  className="bg-gradient-to-br from-primary/5 to-blue-50/30 border border-primary/10 rounded-2xl rounded-tl-sm p-4 text-[15px] text-foreground/90 leading-relaxed shadow-sm animate-fade-up"
                  style={{ animationDuration: "400ms" }}
                >
                  Hi! Ready to start your immigration journey? ✈️ <br />
                  <span className="font-semibold text-primary">
                    I'm here to help you schedule your expert consultation!
                  </span>
                </div>
              )}
              ...
              {step >= 3 && (
                <Button
                  asChild
                  className="w-full bg-primary hover:bg-primary/90 text-white shadow-[0_10px_30px_-5px_rgba(var(--primary-rgb),0.4)] group mt-2 rounded-2xl h-14 animate-fade-up border-b-4 border-primary/40 active:border-b-0 active:translate-y-0.5 transition-all"
                >
